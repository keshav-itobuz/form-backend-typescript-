...
export const storeEmployeeData: RequestHandler = async (req, res, next) => {
    try {
        ...
        res.status(StatusCode.OK).json({ data: null, message: "successfully stored the data" });
    }
    catch (error) {
        next(error);
    }
...
export const deleteEmployeeData = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        res.status(StatusCode.NO_CONTENT).json({ data: null, message: "Employee data deleted successfully" })
    }
    catch (error) {
        next(error);
    }
...
export const deleteAllEmployee = async (req: Request, res: Response, next: NextFunction) => {
    try {
        await User.deleteMany();
        res.status(200).json(StatusCode.NO_CONTENT).json({ data: null, message: "Successfully deleted all the data" })
    }
    catch (error) {
        next(error);
    }
